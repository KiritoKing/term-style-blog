...
- Remediate known dependency vulnerabilities with Astro 7 and compatible integrations, preserving unified Markdown and HTML whitespace behavior; validate the full approved corpus.
...
## Impact
README, LICENSE/NOTICE, contributor/security documents, docs, package.json, pnpm lock/workspace configuration, astro.config.mjs, two fixture ZodError accessors, .env.example, .gitignore, .github configuration, agent records and repository settings. Public exposure includes all reachable Git refs and Actions history; visibility is the final step after review and verification. Do not rewrite history or delete runs unless a concrete sensitive finding requires remediation with a private backup.
