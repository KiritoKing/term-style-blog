...
type TerminalOutput = { type: 'cmd' | 'out' | 'err'; text: string };
...
export default function App() {
  ...
  const getPromptPath = () => {
    ...
    if (route.startsWith('category/')) {
      return `~/${route.replace('category/', 'categories/')}`;
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setBootText((previous: string) => previous + bootSequence[index] + '\n');
      index += 1;
    ...
    if (event.key === 'Tab') {
      ...
      if (parts.length === 1) {
        ...
        if (matches.length === 1) {
          ...
          setTerminalOutput((previous: TerminalOutput[]) => [
            ...previous,
        ...
        parts.length === 2 &&
        (parts[0].toLowerCase() === 'cd' || parts[0].toLowerCase() === 'cat')
      ) {
        ...
        if (parts[0].toLowerCase() === 'cd') {
          if (route === 'home') {
            ...
            const tags = Array.from(
              new Set(POSTS.flatMap((post) => post.tags.map((tag) => tag.toLowerCase()))),
            );
            options = [
              'posts',
              'categories',
            ...
            options = ['~', '..', ...Array.from(new Set(POSTS.map((post) => post.category.toLowerCase())))];
          } else if (route === 'tags') {
            options = [
              '~',
              '..',
          ...
          if (route === 'home') {
            ...
            options = POSTS.filter((post) => post.category.toLowerCase() === cat).map(
              (post) => `${post.id}.md`,
            );
          } else if (route.startsWith('tag/')) {
            const tag = route.split('/')[1];
            options = POSTS.filter((post) =>
              post.tags.map((entry) => entry.toLowerCase()).includes(tag),
            ).map((post) => `${post.id}.md`);
          } else if (route === 'about') {
            options = ['about.txt'];
          } else {
      ...
      if (cmd) {
        setCmdHistory((previous: string[]) => [...previous, cmd]);
        setHistoryIndex(-1);
        setTerminalOutput((previous: TerminalOutput[]) => [
          ...previous,
          { type: 'cmd', text: `guest@server:${getPromptPath()} $ ${cmd}` },
      ...
      if (cmdLower === 'ls' || cmdLower === 'dir') {
        if (route === 'home') {
          ...
          const categories = Array.from(new Set(POSTS.map((post) => post.category.toLowerCase())));
          outputText = categories.map((category) => `${category}/`).join('\n');
        } else if (route === 'tags') {
          const tags = Array.from(
            new Set(POSTS.flatMap((post) => post.tags.map((tag) => tag.toLowerCase()))),
          );
          outputText = tags.map((tag) => `${tag}/`).join('\n');
        } else if (route.startsWith('category/')) {
          const cat = route.split('/')[1];
          outputText = POSTS.filter((post) => post.category.toLowerCase() === cat)
            .map((post) => `${post.id}.md`)
            .join('\n');
        } else if (route.startsWith('tag/')) {
          const tag = route.split('/')[1];
        ...
        if (target === '~' || target === '/') {
          ...
          const tags = Array.from(
            new Set(POSTS.flatMap((post) => post.tags.map((entry) => entry.toLowerCase()))),
          );
        ...
        if (route === 'home') outputText = '/home/guest';
          ...
          outputText = `/home/guest/categories/${route.split('/')[1]}`;
  ...
  const renderContent = () => {
    if (route === 'home') {
      return (
        <div className="animate-in fade-in duration-300">
          ...
          <h1 className="text-4xl font-pixel text-gray-900 dark:text-white mb-4 text-glow-white">
            SYSTEM ONLINE
          </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-lg max-w-2xl mb-8 p-4 border border-gray-300 dark:border-[#333] bg-gray-50 dark:bg-[#0a0a0a]">
            <div className="flex">
              <span className="text-gray-500 w-24">OS:</span>
              <span className="text-blue-600 dark:text-green-300">Debian GNU/Linux 12</span>
            </div>
            <div className="flex">
              <span className="text-gray-500 w-24">Kernel:</span>
              <span className="text-blue-600 dark:text-green-300">6.1.0-18-amd64</span>
            </div>
            <div className="flex">
              <span className="text-gray-500 w-24">Uptime:</span>
              <span className="text-blue-600 dark:text-green-300">94 days, 12:34</span>
            </div>
            <div className="flex">
              <span className="text-gray-500 w-24">Packages:</span>
              <span className="text-blue-600 dark:text-green-300">1423 (dpkg)</span>
            </div>
            <div className="flex">
              <span className="text-gray-500 w-24">Shell:</span>
              <span className="text-blue-600 dark:text-green-300">zsh 5.9</span>
            </div>
            <div className="flex">
              <span className="text-gray-500 w-24">Terminal:</span>
              <span className="text-blue-600 dark:text-green-300">xterm-256color</span>
            </div>
          </div>
          ...
          >
            <h3 className="text-2xl font-pixel text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-green-400 mb-2">
              {POSTS[0].title}
            </h3>
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-2">
              {POSTS[0].date} | {POSTS[0].category} | {POSTS[0].tags.join(', ')}
            </p>
      ...
      return (
        <div className="animate-in fade-in duration-300">
          ...
          <div className="flex flex-col gap-2">
            ...
            {categories.map((cat) => {
              const count = POSTS.filter((post) => post.category.toLowerCase() === cat).length;
              return (
                <div
                  key={cat}
    ...
    if (route === 'posts' || route.startsWith('category/') || route.startsWith('tag/')) {
      ...
      if (route.startsWith('category/')) {
        const cat = route.split('/')[1];
        displayPosts = POSTS.filter((post) => post.category.toLowerCase() === cat);
    ...
    if (route.startsWith('post/')) {
      const post = POSTS.find((entry) => entry.id === route.split('/')[1]);
      ...
      return (
        <div className="animate-in fade-in duration-300">
          ...
          <div className="border border-gray-300 dark:border-[#333] bg-gray-50 dark:bg-[#0a0a0a] p-6 md:p-8 text-gray-700 dark:text-gray-300 relative">
            ...
            <h1 className="text-3xl md:text-5xl font-pixel text-gray-900 dark:text-white mb-4 text-glow-white">
              {post.title}
            </h1>
            <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-8 border-b border-gray-300 dark:border-[#333] pb-4">
              <span className="bg-white dark:bg-[#111] px-2 py-1 rounded border border-gray-200 dark:border-[#222]">
                DATE: {post.date}
              </span>
              <span
...
`}
            ...
            <div className="flex-1">
              <h2 className="text-3xl font-pixel text-gray-900 dark:text-white mb-6 border-b border-gray-300 dark:border-[#333] pb-2">
                ABOUT_ME
              </h2>
              ...
              <p className="text-xl text-gray-700 dark:text-gray-300 mb-4 leading-relaxed">
                I specialize in creating web experiences that don't just look like standard corporate templates. I
                believe the web should be fun, weird, and personal.
              </p>
              <div className="mt-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {['React', 'TypeScript', 'Rust', 'TailwindCSS', 'Pixel Art', 'Linux'].map((skill) => (
                    <span
                      key={skill}
                      className="bg-white dark:bg-[#111] border border-gray-300 dark:border-[#333] px-3 py-1 text-gray-700 dark:text-gray-300 text-lg"
                    >
                      {skill}
                    </span>
                  ))}
  ...
  return (
    <div
      className={`min-h-screen bg-gray-100 dark:bg-[#050505] text-gray-900 dark:text-[#e5e5e5] font-console p-2 md:p-6 flex items-center justify-center relative overflow-hidden ${
        isA11yMode ? '' : 'crt-flicker'
      }`}
...
`}
            ...
            <div className="flex flex-col w-full text-left gap-1 mt-2">
              <button
                ...
                className={`pixel-btn px-3 py-2 flex items-center gap-3 text-lg ${
                  route === 'home'
                    ? 'bg-blue-100 dark:bg-green-900/40 text-blue-600 dark:text-green-400 border-l-2 border-blue-500 dark:border-green-500'
          ...
          <div className="pixel-panel p-4 flex flex-col gap-3">
            ...
            <a
              href="#"
              className="text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-green-400 flex items-center gap-3 text-lg transition-colors"
            >
        ...
        <div className="flex-1 pixel-panel flex flex-col overflow-hidden bg-white dark:bg-[#020202]">
          ...
          <div className="border-t border-gray-300 dark:border-[#333] bg-gray-50 dark:bg-[#0a0a0a] flex flex-col shrink-0">
            <div
              className={`max-h-48 overflow-y-auto flex flex-col gap-1 text-sm md:text-base px-4 md:px-6 ${
                terminalOutput.length > 0 ? 'pt-4 pb-2' : 'pt-4 pb-2'
              }`}
