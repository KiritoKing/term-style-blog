...
export const requirePublicationString = (
  value: unknown,
  field: string,
  file: string,
): string => {
...
export const isPublicationCategory = (value: string): boolean =>
  value.trim() !== '' && value.trim().toLocaleLowerCase('en-US') !== 'uncategorized';

export const requirePublicationCategory = (value: unknown, file: string): string => {
  const category = requirePublicationString(value, 'category', file);
  if (!isPublicationCategory(category)) {
    throw new Error(`${file}: required field category must not be placeholder Uncategorized`);
  }
  return category;
};
...
export const validatePublicationRecords = (
  ...
  const normalized = records.map((record) => {
    const title = requirePublicationString(record.data.title, 'title', record.path);
    const slug = requirePublicationString(record.data.slug, 'slug', record.path);
    requirePublicationCategory(record.data.category, record.path);
    requirePublicationString(record.data.summary, 'summary', record.path);
